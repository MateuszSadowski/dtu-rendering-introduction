...
//        return;
            ...
            case 4:  // transparent materials
            {
                ...
                float frenelReflectance;
                trace_refracted(r, hit, new_r, new_hit, frenelReflectance);
                if (mt_random() < frenelReflectance) {
                    new_hit.has_hit = false;
                    trace_reflected(r, hit, new_r, new_hit);
                }
                if (!new_hit.has_hit) {
                    return;
                }
